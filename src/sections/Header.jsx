    ...
    <header className='bg-themeblack py-4 lg:px-[100px] px-7 fixed top-0 left-0 w-full z-50'>
      <div className='flex justify-between items-center'>
        <div className='text-themered font-bold text-left text-4xl'>
          NIMSHI <span className='text-white'>.</span>
        </div>
        ...
        <nav className='hidden md:block'>
          <ul className='flex space-x-10 text-lg font-poppins text-white'>
            {menuLinks.map((link) => (
              <li key={link.to}>
                <Link to={link.to} smooth={true} duration={500} className={`cursor-pointer ${link.name === 'Home' ? 'text-themered': 'hover:text-themered transition-colors'}`}>{link.name}</Link>
              </li>
            ))}
          </ul>
        </nav>
      ...
      <div className={`fixed top-0 left-0 h-full w-full bg-black text-white transform ${menuOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 ease-in-out z-40 flex items-center justify-center overflow-y-hidden`}>
        <IoCloseSharp className='fixed top-3 right-6 text-3xl cursor-pointer' onClick={() => setMenuOpen(false)} />
          ...
          <ul>
            {menuLinks.map((link) => (
              <li key={link.to}>
                <Link to={link.to} smooth={true} duration={500} className={`cursor-pointer ${link.name === 'Home' ? 'text-themered': 'hover:text-themered transition-colors font-poppins'}`} onClick={() => setMenuOpen(false)}>{link.name}</Link>
              </li>
            ))}
          </ul>
